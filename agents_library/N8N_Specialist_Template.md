...
**Response node rule:** Never reference Postgres or database node output fields in a response node without confirming the node is configured to return the inserted row. This is version-dependent and unreliable by default. For response nodes that need a known status value, use a hardcoded value per branch rather than reading dynamic output from the DB node. One dedicated response node per terminal branch is always safer than a shared response node reading `$json.status` from upstream.
...
**Connection type rule:** HTTP Request, Postgres, and most action nodes produce one `"main"` output and one `"error"` output. Only IF nodes produce multiple `"main"` outputs — one per branch. Never route a failure path via a second `"main"` entry on an HTTP or database node — use the `"error"` connection key instead. Using two `"main"` outputs on an HTTP Request node will produce an invalid workflow that silently drops the error branch.
...
- **Startup validation pattern:** Check each required env var exists and call `process.exit(1)` (TS) or `sys.exit(1)` (Python) before any service is instantiated. Never use `|| ''` or `or ''` fallbacks — these bypass validation and defer failures to request time. The correct pattern is: check env var → exit if missing → pass to constructor.
- **Error typing:** All catch blocks must use `error: unknown` (TS), not `error: any`. Use `axios.isAxiosError(error)` and `error instanceof Error` type guards to extract error details safely.
